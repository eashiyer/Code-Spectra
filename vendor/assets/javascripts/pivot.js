...
(function() {
  ...
  */
  ...
  PivotData = (function() {
    function PivotData(aggregator, colAttrs, rowAttrs, col_totals, col_dims) {
      ...
      this.flatColKeys = [];
      this.rowTotals = {};
      this.colTotals = col_totals;
      this.allTotal = this.aggregator(this, [], []);
      this.sorted = true;
      this.columnDimensions = col_dims;
    }
    ...
    PivotData.prototype.sortKeysByFormat = function(keys){
      var weekdays = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
      var col_dims = this.columnDimensions;
      // var i = 0;
      // var dim = col_dims[i];
      var totals_entry= _.filter(keys, function(key){
        return key[0]=="Totals";
      });
      if(totals_entry !== undefined && totals_entry.length > 0){
        _.each(totals_entry, function(total_entry){
          keys.removeObject(total_entry);
        });      
      }
      for(i=col_dims.length-1; i>=0; i--){
        var dim = col_dims[i];
        keys = _.sortBy(keys, function(key){
          // if(key[i] != "Totals"){
            if(dim.get('formatAs')){
              switch (dim.get('formatAs').toLowerCase()) {
                case "year":
                  return d3.time.format("%Y").parse(key[i]);
                  break;
                case "month year":
                  return d3.time.format("%Y %b").parse(key[i]);
                  break;
                case "month":
                  return d3.time.format("%b").parse(key[i]);
                  break;
                case "quarter":
                  return d3.time.format("%YQ%m").parse(key[i]);
                  break;
                case "day":
                  return weekdays.indexOf(key[i]);
                  break;
                case "week":
                  return parseInt(key[i].split("W")[1]);
                  break;
                default:
                  return key[i];
                  break;
        ...
        if(dim.get('sortOrder') && dim.get('sortOrder').toLowerCase() == "desc"){
          keys.reverse();
        }
      } 
      if(totals_entry !== undefined && totals_entry.length > 0){
        _.each(totals_entry, function(total_entry){
          keys.push(total_entry);
        });
  ...
  getPivotData = function(input, cols, rows, aggregator, filter, derivedAttributes, col_totals, col_dims) {
    var pivotData;
    pivotData = new PivotData(aggregator, cols, rows, col_totals, col_dims);
  ...
  pivotTableRenderer = function(pivotData, opts, chart) {
    ...
    if(colAttrs[0] !== "Measure"){
      colKeys = pivotData.sortKeysByFormat(colKeys);
    }
    ...
    for (j in colAttrs) {
      ...
      tr.append($("<th class='pvtAxisLabel'>").text((c != "null")? c : ""));
      for (i in colKeys) {
        if (!__hasProp.call(colKeys, i)) continue;
        colKey = colKeys[i];
        x = spanSize(colKeys, parseInt(i), parseInt(j));
        if (x !== -1) {
          th = $("<th class='pvtColLabel'>").text((colKey[j] != "null")? colKey[j] : "").attr("colspan", x);
          if (parseInt(j) === colAttrs.length - 1 && rowAttrs.length !== 0) {
            th.attr("rowspan", 2);
          }
          if(colKey[0] == "Totals"){
            th.addClass("totalCell");
          }else{
            var dimensionDisplayName = colAttrs[j];
            var dimensionValue = colKey[j];
            if(dimensionDisplayName!="Measure"){
              th.addClass("clickableCells");
              th.on("click", function(key, value){
                // apply onclick filter on th
                return function() {
                  if(value != "null"){
                    value_display = value;
                    if (d3.time.format("%Y %b").parse(value) != null) {
                      value = d3.time.format("%Y %m")(d3.time.format("%Y %b").parse(value));
                    } else if (d3.time.format("%b").parse(value) != null) {
                      value = d3.time.format("%m")(d3.time.format("%b").parse(value));
                    }
                    var id = $(this).closest("table").parent()[0].id + "-col-" + value;
                    $(this).attr("id", id);
                    chart.createChartElementOnClickTooltip(key, value, this.id, event.pageX, event.pageY, value_display);
                    console.log("element_id :: "+this.id);
                    console.log(key + " - " + value);
                    console.log(chart.id);
                  }                  
                };
              }(dimensionDisplayName, dimensionValue));
            }
          }
          tr.append(th);
        }
    ...
    if (rowAttrs.length !== 0) {
      ...
      for (i in rowAttrs) {
        ...
        r = rowAttrs[i];
        tr.append($("<th class='pvtAxisLabel'>").text((r != "null") ? r : ""));
      }
      thead.append(tr);
      result.append(thead);
      th = $("<th>");
      if (colAttrs.length === 0) {
        th.addClass("pvtTotalLabel totalCell").text(opts.localeStrings.totals);
      }
      tr.append(th);
      // result.append(tr);
    }
    for (i in rowKeys) {
      ...
      for (j in rowKey) {
        ...
        if (x !== -1) {
          th = $("<th class='pvtRowLabel'>").text((txt != "null") ? txt : "").attr("rowspan", x);
          if (parseInt(j) === rowAttrs.length - 1 && colAttrs.length !== 0) {
            th.attr("colspan", 2);
          }
          var dimensionDisplayName = rowAttrs[j];
          var dimensionValue = rowKey[j];
          if(dimensionDisplayName!="Measure"){
            th.addClass("clickableCells");
            th.on("click", function(key, value){              
              // apply onclick filter on th
              return function() {
                if(value != "null"){
                  ...
                  var id = $(this).closest("table").parent()[0].id + "-row-" + value;
                  $(this).attr("id", id);
                  chart.createChartElementOnClickTooltip(key, value, this.id, event.pageX, event.pageY, value_display);
                  console.log("element_id :: "+this.id);
                  console.log(key + " - " + value);
      ...
      for (j in colKeys) {
        ...
        colKey = colKeys[j];
        var measureClass = colKey[colKey.length-1].replace(/\s/g,"");
        aggregator = pivotData.getAggregator(rowKey, colKey);
        val = aggregator.value();
        if(colKey[0]=="Totals"){
          tr.append($("<th class='pvtVal totalCell rightAligned row" + i + " col" + j + " "+measureClass+"'>").html(aggregator.format(val)).data("value", val));
        }else{
          tr.append($("<td class='pvtVal row" + i + " col" + j + " "+measureClass+"'>").html(aggregator.format(val)).data("value", val));
        }        
      }
      // totalAggregator = pivotData.getAggregator(rowKey, []);
      // val = totalAggregator;
      // tr.append($("<td class='pvtTotal rowTotal'>").html(val).data("value", val).data("for", "row" + i));
      result.append(tr);
    }
    tr = $("<tr>");
    th = $("<th class='pvtTotalLabel totalCell'>").text(opts.localeStrings.totals);
    ...
    for (j in colKeys) {
      ...
      val = totalAggregator;
      tr.append($("<td class='pvtTotal colTotal totalCell "+measureClass+"'>").html(val).data("value", val).data("for", "col" + j));
    }
    // totalAggregator = pivotData.getAggregator([], []);
    // val = totalAggregator.value();
    // tr.append($("<td class='pvtGrandTotal'>").html(totalAggregator.format(val)).data("value", val));
    // // val = totalAggregator;
    // // tr.append($("<td class='pvtGrandTotal'>").html(val).data("value", val));
    result.append(tr);
  ...
  */
  ...
  $.fn.pivot = function(input, opts) {
    ...
    result = null;
    try {
      pivotData = getPivotData(input, opts.cols, opts.rows, opts.aggregator, opts.filter, opts.derivedAttributes, opts.col_totals, opts.col_dims);
      try {
        result = opts.renderer(pivotData, opts.rendererOptions, opts.chart);
      } catch (_error) {
        e = _error;
  ...
  */
  ...
  $.fn.heatmap = function(table_data, displayUnit, scope){

    _.each(table_data, function(d){
        var val = d.Measure_Val;
        if(val && val.trim().length > 0){

          //if prefix exists, remove it from the value
          if(val.indexOf("$") != -1){
            val = val.replace("$","");
          ...
          //if suffix exists, remove it from the value
          if(val.indexOf("%") != -1){
            val = val.replace("%","");
          }
          ...
          //if display unit exists, remove the formatting from value
          if(displayUnit){
            if(val.substr(val.length -1) == "k"){
              val = "" + revrseFormatting(parseFloat(val),"k") + "";
            }else if(val.substr(val.length -1) == "L"){
              val = "" + revrseFormatting(parseFloat(val),"L") + "";
            }else if(val.substr(val.length -1) == "M"){
              val = "" + revrseFormatting(parseFloat(val),"M") + "";
            }else if(val.substr(val.length -2) == "Cr"){
              val = "" + revrseFormatting(parseFloat(val),"Cr") + "";
            }else if(val.substr(val.length -1) == "B"){
              val = "" + revrseFormatting(parseFloat(val),"B") + "";
            } 
          }
        }         
        d.Measure_Val = val.trim();
      });
    ...
    var measureClasses = table_data.map(function(d){
                            return d.Measure.replace(/\s/g,"");
                          });
    measureClasses = measureClasses.uniq();

    colorGen = function(x, measure){
      var dat = _.filter(table_data, function(d){
        return (d.Measure.replace(/\s/g,"") == measure);
      });

      var min = d3.min(dat, function (d) { 
        return d.Measure_Val;
      });
    ...
    heatmapper = function(scope, color) {
      ...
      forEachCell = function(f) {
        return _this.find(scope).each(function() {
          ...
          var mesClass = _.intersection(class_arr, measureClasses);
          ...
          if(x && x.trim().length > 0){
            ...
            //if display unit exists, remove the formatting from value
            if(displayUnit){
              if(x.substr(x.length -1) == "k"){
                x = "" + revrseFormatting(parseFloat(x),"k") + "";
              }else if(x.substr(x.length -1) == "L"){
                x = "" + revrseFormatting(parseFloat(x),"L") + "";
              }else if(x.substr(x.length -1) == "M"){
                x = "" + revrseFormatting(parseFloat(x),"M") + "";
              }else if(x.substr(x.length -2) == "Cr"){
                x = "" + revrseFormatting(parseFloat(x),"Cr") + "";
              }else if(x.substr(x.length -1) == "B"){
                x = "" + revrseFormatting(parseFloat(x),"B") + "";
              } 
            }
          } 

          if ((x != null) && isFinite(x)) {
            return f(x, $(this), mesClass);
          }
        });
      };
      values = [];
      ...
      colorFor = colorGen;
      return forEachCell(function(x, elem, mesClass) {
        return elem.css("background-color", colorFor(x, mesClass));
      });
    };
    switch (scope) {
      case "heatmap":
        heatmapper(".pvtVal", "green");
        break;
      case "rowheatmap":
        for (i = _i = 0; 0 <= numRows ? _i < numRows : _i > numRows; i = 0 <= numRows ? ++_i : --_i) {
          heatmapper(".pvtVal.row" + i, "green");
        }
        break;
      case "colheatmap":
        for (j = _j = 0; 0 <= numCols ? _j < numCols : _j > numCols; j = 0 <= numCols ? ++_j : --_j) {
          heatmapper(".pvtVal.col" + j, "green");
        }
    }
    heatmapper(".pvtTotal.rowTotal", "green");
    heatmapper(".pvtTotal.colTotal", "green");
    return this;
  };
