(function(global) {
	 ...
	 *                                      data counts.
	 */
	D3Funnel.prototype.draw = function(data, options,obj,tip,total_val,cds) {
		...
		for (var i = 0; i < sectionPaths.length; i++) {
			...
			path = group.append("path")
				.attr("fill", fill)
				.attr("d", pathStr)
				.attr("stroke", d3.rgb(this.data[i][2]).darker() )
				.data([data]);
			...
			if (this.hoverEffects) {
				path.on("mouseover", function(d){
						if(!obj.chart.get('notToHideTip')){
							d.key = this.__data__['label'];
		                    d.value = this.__data__['value']
		                    var otherData={};
					...
					.on("click", function(d){
						var id = "chart-" + obj.chart.id + "-" + pruneStr(d['label']); 
						var filter = {};
	                    filter.dimension = cds.get('dimensionName');
              			filter.formatAs = cds.get('dimensionFormatAs') ? cds.get('dimensionFormatAs') : "";
                      	filter.value = d['label'];
                      	d.key = d['label'];
                      	d.value = d['value']
                      	var otherData={};
                      	otherData.total_val=total_val;
                      	// tip.offset([10,0]);
                      	obj.chart.chartElementOnClick(id, filter, d, otherData, tip, svg);
					})
	...
	D3Funnel.prototype._initialize = function(data, options) {
		...
		for (i = 0; i < this.data.length; i++) {
			...
			if (typeof this.data[i][2] === "undefined" ||
				...
				this.data[i][2] = colorScale(this.data[i][0]);
			}  // End if
		...
		for (var i = 0; i < this.data.length; i++) {
			...
			if (this.isCurved) {
				paths.push([
					...
					// Bottom Bezier curve
					[nextRightX, nextHeight, "L"],
					[middle, nextHeight + this.curveHeight -10 , "Q"],
	 ...
	 * @param {Object} data
	 */
	D3Funnel.prototype._onMouseOver = function(data,obj) {
		var svgElement = this.parentNode.parentNode.parentNode;
		var tip = d3.tip()
    			 .attr('class','d3-tip');
